...
/*
 * mpt_domain_finalized(): Build the MPT table of a supervisor domain.
 *
...
static int mpt_domain_finalized(struct sbi_domain *dom, void *priv)
{
	int rc;
	u8 base_xwr;
	...
	struct sbi_mpt_domain *sd = sbi_domain_state_ptr(dom, &mpt_domain_state);
	struct sbi_mpt_ctrl *ctrl = &mpt_ctrl;
			      ...
			      struct sbi_domain_state *state, void *state_ptr)
{
	 ...
	 * allocated by mpt_domain_finalized(). A recursive free is not
...
int sbi_mpt_init(void)
{
	...
	if (rc) {
		sbi_printf("sbi_mpt: domain state register failed rc=%d\n", rc);
		mpt_ctrl.ready = false;
		return rc;
	}

	/*
	 * The MPT table of a domain is built from its memory regions, which
	 * are only final once every domain is registered, so it is done from
	 * the domain finalize notifier and not from state_setup().
	 */
	rc = sbi_domain_register_finalize_notifier(mpt_domain_finalized, NULL);
	if (rc) {
		sbi_printf("sbi_mpt: finalize notifier register failed rc=%d\n",
			   rc);
		sbi_domain_unregister_state(&mpt_domain_state);
		mpt_ctrl.ready = false;
		return rc;
	}
